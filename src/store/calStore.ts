...
export const useCalculator = defineStore("calculator", {
  ...
  actions: {
    loanCalc(val: CalFormVal) {
      ...
      //簽約金
      this.sign = toMoneyStyle(Math.ceil(val.total * 0.12 - 20));
      //1樓底板
      ...
      //結構完成
      this.construction = toMoneyStyle(Math.ceil(val.total * 0.01));
      //交屋款
      this.delivery = toMoneyStyle(Math.ceil(val.total * 0.05));

      //---------------- 拆款 END ----------------
      //自備款
      this.ownMoney = toMoneyStyle(Math.ceil(val.total * 0.3));
      //貸款
      this.loanMoney = toMoneyStyle(Math.floor(val.total * 0.7));
      ...
      this.monthlyCost = toMoneyStyle(
        Math.floor(val.total * 0.7 * avgMonthRatio * 10000)
      );
      //還息金額
      this.interestRepayment = toMoneyStyle(
        (totalMoney - val.total * 0.7 * 10000) / loanMonth
      );
      //寬限期
